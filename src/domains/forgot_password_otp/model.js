 ...
 */
const createPasswordResetOTP = async ({ userId, otp, createdAt, expiresAt }) => {
...
const getPasswordResetOTPByUserId = async (userId) => {
  const otpQuery = db.collection("passwordResetOTPs").where("userId", "==", userId).limit(1);
...
const deletePasswordResetOTPsByUserId = async (userId) => {
  const otpQuery = db.collection("passwordResetOTPs").where("userId", "==", userId);
  const otpSnapshot = await otpQuery.get();
