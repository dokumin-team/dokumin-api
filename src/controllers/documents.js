...
module.exports.getCountDocumentsInAllFolders = async (req, res, next) => {
    try {
        ...
        const foldersSnapshot = await db.collection(`users/${userId}/folders`).get();

        let totalCount = 0;

        const folderPromises = foldersSnapshot.docs.map(async (folderDoc) => {
            const folderId = folderDoc.id;
            const documentsSnapshot = await db.collection(`users/${userId}/folders/${folderId}/documents`).get();
...
module.exports.getSearchDocument = async (req, res, next) => {
    try {
        ...
        const folderPromises = foldersSnapshot.docs.map(async (folderDoc) => {
            const folderId = folderDoc.id;
            const snapshot = await db.collection(`users/${userId}/folders/${folderId}/documents`).get();
...
module.exports.updateDocument = async (req, res, next) => {
    try {
        ...
        const docRef = db.collection(`users/${userId}/folders/${folderId}/documents`).doc(documentId);
