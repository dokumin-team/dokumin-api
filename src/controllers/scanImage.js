...
module.exports.scanImageToFolder = async (req, res, next) => {
    try {
        ...
        // Kirim ke Flask API
        const flaskResponse = await axios.post(`${process.env.FLASK_API_URL}/process-image`, { file });
        ...
        // Upload ke GCS
        const stream = blob.createWriteStream({ metadata: { contentType: 'application/pdf' } });
        stream.end(buffer);
        ...
        stream.on('finish', async () => {
            ...
            const folderRef = db.collection(`users/${userId}/folders/${folderName}/documents`);
            ...
            res.status(201).json({ success: true, documentId: doc.id, url: publicUrl });
        });
